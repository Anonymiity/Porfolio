    ...
    >
      <motion.div
        ...
        style={{
          ...
          boxShadow: isActive
            ? `0 0 40px ${planet.color}, 0 0 80px ${planet.color}60, inset 0 0 20px rgba(255,255,255,0.3)`
            : `0 0 20px ${planet.color}80, inset 0 0 10px rgba(255,255,255,0.2)`,
      ...
      >
        ...
        {/* Level badge */}
        <div
          className="absolute -top-3 flex h-5 items-center justify-center rounded-full border border-white/20 bg-black/60 px-1.5 text-[9px] font-black text-white backdrop-blur-sm"
        >
          L{planet.level}
        </div>
      </motion.div>
...
function Asteroid({ name, angle, radius, size, speed }) {
  ...
  return (
    ...
    >
      <div
        className="rounded-full bg-gray-400/40 transition-all duration-300 group-hover:bg-cyan-300/80 group-hover:shadow-lg group-hover:shadow-cyan-400/50"
        style={{ width: size, height: size }}
        title={name}
      />
      <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap text-[9px] font-mono text-cyan-300 opacity-0 transition-opacity group-hover:opacity-100 pointer-events-none bg-black/80 px-1.5 py-0.5 rounded">
        {name}
      </span>
    </div>
  );
...
function CentralStar() {
  ...
  return (
    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none">
      {/* Outer glow rings */}
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-40 w-40 rounded-full bg-orange-400/10 blur-3xl animate-pulse" />
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-24 w-24 rounded-full bg-cyan-400/20 blur-2xl" />

      {/* Core */}
      <div className="relative flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-orange-300 via-amber-300 to-yellow-200 shadow-[0_0_60px_rgba(251,191,36,0.8)]">
        <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-white/40 to-transparent" />
        <span className="text-xs font-black text-black/70">TL</span>
      </div>

      {/* Name ring */}
      <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 whitespace-nowrap text-center">
        <p className="text-sm font-bold tracking-widest text-white">{star.name}</p>
        <p className="text-[10px] uppercase tracking-[0.3em] text-cyan-400">{star.class}</p>
      </div>
    </div>
  );
}
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute right-4 top-4 rounded-full p-1 text-gray-400 transition hover:text-white"
          >
            <X size={18} />
          </button>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-xs text-gray-400">Level {planet.level}</span>
              </div>
            </div>
          </div>
          ...
          {/* Moons (tools) */}
          <div className="mt-5">
            <p className="text-[10px] uppercase tracking-[0.3em] text-gray-500 mb-2">Orbiting Tools</p>
            <div className="flex flex-wrap gap-2">
              {planet.moons.map((moon) => (
                <span
                  key={moon}
                  className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-mono text-cyan-200"
                >
                  {moon}
                </span>
              ))}
    ...
    <div className="absolute inset-x-0 top-0 z-30 pointer-events-none px-6 py-6">
      <div className="mx-auto flex max-w-6xl justify-between">
        ...
        <div className="pointer-events-auto">
          ...
          <p className="text-xs text-gray-400">{star.class} &bull; L{star.level}</p>
        </div>
          ...
          <div className="flex gap-4">
            {systems.map((sys) => (
              <div key={sys.name} className="text-right">
                ...
                <p className="text-xs font-mono text-gray-300">{sys.name}</p>
...
function BottomControls({ activeFilter, setFilter }) {
  ...
  return (
    <div className="absolute bottom-0 inset-x-0 z-30 flex justify-center pb-8">
      <div className="flex gap-2 rounded-full border border-white/10 bg-black/40 p-2 backdrop-blur-xl">
        {filters.map((f) => (
          <button
            key={f}
        ...
        return (
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-400">L{planet.level}</span>
                {isExpanded ? <ChevronUp size={18} className="text-cyan-400" /> : <ChevronDown size={18} className="text-gray-500" />}
              </div>
            </button>
...
export default function Skills() {
  ...
  return (
    <section
      id="skills"
      className="relative min-h-screen overflow-hidden bg-[#050816] text-white py-20 md:py-0 md:h-screen"
    >
      {/* Background layers */}
      ...
      <div className="md:hidden bottom-4 left-0 right-0 z-40 flex justify-center">
        <div className="flex gap-2 rounded-full border border-white/10 bg-black/80 p-2 backdrop-blur-xl">
      ...
      <div className="absolute bottom-8 right-8 z-30 pointer-events-none hidden md:block text-right">
        ...
        <div className="mt-1 text-xs font-mono text-cyan-400/60">
